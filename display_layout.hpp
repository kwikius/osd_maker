...
struct display_layout{
   ...
   void arc_out(uint32_t  radius, quan::angle::deg const & start_angle,quan::angle::deg const & end_angle, uint32_t numsegs, colour c);
  ...
  // bool set_background(wxString const & filename);
   ...
   private:
   void normalised_arc(uint32_t radius, quan::angle::deg const & start_angle,
