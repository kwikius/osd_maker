...
void display_layout::normalised_arc(uint32_t radius, quan::angle::deg const & start_angle,
      quan::angle::deg const & end_angle, uint32_t numsegs, colour c)
{
   ...
   for ( auto i = 0U; i < numsegs; ++i){
         ...
         auto angle2 = start_angle + angle_inc * (i+1);
         auto c1 = cos(angle1);
         auto s1 = sin(angle1);
         auto c2 = cos(angle2);
         auto s2 = sin(angle2);
...
void display_layout::arc_out(uint32_t radius, quan::angle::deg const & start_angle_in,
      quan::angle::deg const & end_angle_in, uint32_t numsegs, colour c)
{
      ...
      if ( end_angle > start_angle){
         normalised_arc(radius,start_angle, end_angle,numsegs, c);
      }else{
         ...
         normalised_arc(radius, start_angle, quan::angle::deg{360}, first_segs,c);
         normalised_arc(radius, quan::angle::deg{0},end_angle, last_segs,c);
      }
}    
