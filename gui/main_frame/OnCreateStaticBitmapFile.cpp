...
namespace {
   ...
   uint8_t remap_pixel_aerflite(uint8_t in)
   {
       switch (in){
          case  0b00:
             return 0b01;
...
void main_frame::OnCreateStaticBitmapFile(wxCommandEvent & event)
{
   ...
   boards.Add(wxT("aerflite"));
   boards.Add(wxT("quantracker"));

   wxSingleChoiceDialog board_dialog{
      this
      ,wxT("Choose Board")
      ,wxT("Bitmapss to C++ sources/headers")
      ,boards
   };
   if (board_dialog.ShowModal() != wxID_OK){
     return;
   }
   int board_index = board_dialog.GetSelection();
   ...
   if (dlg.ShowModal() == wxID_OK){
      ...
      for ( size_t i = 0; i < result.GetCount();++i){
         ...
         if ( bmp ){
            ...
            if ( board_index == 0){
                 auto * cloned = bmp->clone();
                 cloned->pixel_remap(remap_pixel_aerflite);
                 cloned->output_header( bmp_name1 + "_type", bmp_name1,out);
                 delete cloned;
            }else{
